...
submit_frame=function(model,train_start,test_start,test_end){
  #读取数据---------------------------------------------------------------
  ...
  result <- result[result$sale_date != "20150910",]
  names(result) = c("bianma","riqi","xiaoliang")
  result$riqi=as.integer(as.character(result$riqi))
